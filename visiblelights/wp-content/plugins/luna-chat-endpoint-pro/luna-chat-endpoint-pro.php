...
/* =========================================================================
 * CORS Headers for Luna Widget Endpoints
 * ========================================================================= */

/**
 * Add CORS headers for Luna Widget endpoints
 */
// Handle OPTIONS requests early, before WordPress processes them
add_action('init', function() {
  if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'OPTIONS' && 
      isset($_SERVER['REQUEST_URI']) &&
      (strpos($_SERVER['REQUEST_URI'], '/wp-json/luna_widget/') !== false || 
       strpos($_SERVER['REQUEST_URI'], '/wp-json/luna/v1/') !== false)) {
    ...
    if ($origin !== '*' && !empty($origin)) {
      foreach ($allowed_origins as $allowed) {
        ...
        if ($allowed_host && $origin_host && ($origin_host === $allowed_host || substr($origin_host, -strlen($allowed_host)) === $allowed_host)) {
          $origin_allowed = true;
          break;
        }
      }
    ...
    header('Access-Control-Allow-Headers: Content-Type, X-Luna-License, x-luna-license, Authorization, Accept');
...
add_action('rest_api_init', function() {
  // Add CORS headers using WordPress's built-in filter
  add_filter('rest_send_cors_headers', function($value, $request) {
    $route = $request->get_route();
    
    // Only apply to Luna Widget endpoints
    if (strpos($route, '/luna_widget/') === 0 || strpos($route, '/luna/v1/') === 0) {
      return true; // Enable CORS for these routes
    }
    
    return $value;
  }, 10, 2);
  
  // Add custom headers to CORS allow list (case-insensitive)
  add_filter('rest_allowed_cors_headers', function($headers) {
    if (!is_array($headers)) {
      $headers = [];
    }
    $headers[] = 'X-Luna-License';
    $headers[] = 'x-luna-license'; // Lowercase version for browser compatibility
  ...
  // Add CORS headers for all requests to Luna Widget endpoints (high priority to run before WordPress defaults)
  add_filter('rest_pre_serve_request', function($served, $result, $request, $server) {
    ...
    if (strpos($route, '/luna_widget/') === 0 || strpos($route, '/luna/v1/') === 0) {
      ...
      $origin = isset($_SERVER['HTTP_ORIGIN']) ? $_SERVER['HTTP_ORIGIN'] : (isset($_SERVER['HTTP_REFERER']) ? parse_url($_SERVER['HTTP_REFERER'], PHP_URL_SCHEME) . '://' . parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST) : '*');
      ...
      if ($origin !== '*' && !empty($origin)) {
        foreach ($allowed_origins as $allowed) {
          ...
          // Subdomain match (e.g., https://supercluster.visiblelight.ai matches visiblelight.ai)
          $allowed_host = parse_url($allowed, PHP_URL_HOST);
          $origin_host = parse_url($origin, PHP_URL_HOST);
          if ($allowed_host && $origin_host && ($origin_host === $allowed_host || substr($origin_host, -strlen($allowed_host)) === $allowed_host)) {
            $origin_allowed = true;
    ...
    return $served;
  }, 1, 4); // Priority 1 to run before WordPress default CORS handling
});
...
add_action('rest_api_init', function() {
  add_filter('rest_pre_dispatch', function($result, $server, $request) {
    ...
    // Handle OPTIONS preflight for Luna Widget endpoints
    if ((strpos($route, '/luna_widget/') === 0 || strpos($route, '/luna/v1/') === 0) && 
...
/**
 * Format VL Hub profile data for chat context
 */
function luna_format_profile_for_chat(array $profile): string {
  $context_parts = [];
  
  // Site Information
  if (!empty($profile['site'])) {
    $site = $profile['site'];
    $context_parts[] = "SITE INFORMATION:";
    if (!empty($site['home_url'])) {
      $context_parts[] = "- Home URL: " . $site['home_url'];
    }
    if (isset($site['https'])) {
      $context_parts[] = "- HTTPS Enabled: " . ($site['https'] ? 'Yes' : 'No');
    }
    if (isset($profile['https'])) {
      $context_parts[] = "- HTTPS Status: " . ($profile['https'] ? 'Yes' : 'No');
    }
  }
  
  // SSL/TLS Information
  if (!empty($profile['security']['tls']) || !empty($profile['ssl_tls'])) {
    $tls = $profile['security']['tls'] ?? $profile['ssl_tls'] ?? [];
    $context_parts[] = "\nSSL/TLS CERTIFICATE STATUS:";
    if (!empty($tls['status'])) {
      $context_parts[] = "- SSL Status: " . $tls['status'];
    }
    if (!empty($tls['version'])) {
      $context_parts[] = "- TLS Version: " . $tls['version'];
    }
    if (!empty($tls['issuer'])) {
      $context_parts[] = "- Certificate Issuer: " . $tls['issuer'];
    }
    if (!empty($tls['valid_from'])) {
      $context_parts[] = "- Valid From: " . $tls['valid_from'];
    }
    if (!empty($tls['valid_to'])) {
      $context_parts[] = "- Valid To: " . $tls['valid_to'];
    }
    if (!empty($tls['host'])) {
      $context_parts[] = "- Certificate Host: " . $tls['host'];
    }
  } elseif (isset($profile['https']) && $profile['https']) {
    $context_parts[] = "\nSSL/TLS CERTIFICATE STATUS:";
    $context_parts[] = "- HTTPS is enabled (SSL certificate is active)";
  }
  
  // WordPress Information
  if (!empty($profile['wordpress'])) {
    $wp = $profile['wordpress'];
    $context_parts[] = "\nWORDPRESS INFORMATION:";
    if (!empty($wp['version'])) {
      $context_parts[] = "- WordPress Version: " . $wp['version'];
    }
    if (!empty($wp['theme']['name'])) {
      $context_parts[] = "- Active Theme: " . $wp['theme']['name'];
      if (!empty($wp['theme']['version'])) {
        $context_parts[] = "- Theme Version: " . $wp['theme']['version'];
      }
    }
  }
  ...
  if (!empty($profile['plugins']) && is_array($profile['plugins'])) {
    $plugin_count = count($profile['plugins']);
    $active_count = 0;
    foreach ($profile['plugins'] as $plugin) {
      if (is_array($plugin) && (!empty($plugin['is_active']) || (!empty($plugin['status']) && stripos($plugin['status'], 'active') !== false))) {
        $active_count++;
      }
    }
    $context_parts[] = "\nPLUGINS:";
    $context_parts[] = "- Total Plugins: " . $plugin_count;
    $context_parts[] = "- Active Plugins: " . $active_count;
  }
  ...
  if (!empty($profile['connections'])) {
    ...
    if (!empty($connections['ssl_tls'])) {
      $context_parts[] = "- SSL/TLS: Connected";
    }
    if (!empty($connections['cloudflare'])) {
      $context_parts[] = "- Cloudflare: Connected";
      if (!empty($profile['cloudflare']['zones_count'])) {
        $context_parts[] = "  - Zones: " . $profile['cloudflare']['zones_count'];
      }
    }
    if (!empty($connections['aws_s3'])) {
      $context_parts[] = "- AWS S3: Connected";
      if (!empty($profile['aws_s3']['bucket_count'])) {
        $context_parts[] = "  - Buckets: " . $profile['aws_s3']['bucket_count'];
      }
    }
    if (!empty($connections['ga4'])) {
      $context_parts[] = "- Google Analytics 4: Connected";
      if (!empty($profile['ga4']['property_id'])) {
        $context_parts[] = "  - Property ID: " . $profile['ga4']['property_id'];
      }
    }
    if (!empty($connections['liquidweb'])) {
      $context_parts[] = "- Liquid Web: Connected";
      if (!empty($profile['liquidweb']['assets_count'])) {
        $context_parts[] = "  - Assets: " . $profile['liquidweb']['assets_count'];
      }
    }
    if (!empty($connections['gsc'])) {
      $context_parts[] = "- Google Search Console: Connected";
    }
  ...
  if (!empty($profile['client_streams']) && is_array($profile['client_streams'])) {
    $active_streams = [];
    foreach ($profile['client_streams'] as $stream_id => $stream) {
      if (is_array($stream) && empty($stream['status']) || (!empty($stream['status']) && $stream['status'] !== 'inactive')) {
        $active_streams[] = $stream;
      }
    }
    if (!empty($active_streams)) {
      $context_parts[] = "\nDATA STREAMS (" . count($active_streams) . " active):";
      foreach (array_slice($active_streams, 0, 10) as $stream) {
        $name = $stream['name'] ?? $stream['stream_name'] ?? 'Unnamed Stream';
        $categories = !empty($stream['categories']) && is_array($stream['categories']) ? implode(', ', $stream['categories']) : 'uncategorized';
        $context_parts[] = "- " . $name . " (" . $categories . ")";
      }
      if (count($active_streams) > 10) {
        $context_parts[] = "- ... and " . (count($active_streams) - 10) . " more";
      }
    }
  }
  ...
  if (!empty($profile['security'])) {
    ...
    if (!empty($security['waf'])) {
      $has_security_data = true;
      if (!empty($security['waf']['provider'])) {
        $security_parts[] = "- WAF Provider: " . $security['waf']['provider'];
      }
    }
    
    if (!empty($security['ids'])) {
      $has_security_data = true;
      if (!empty($security['ids']['provider'])) {
        $security_parts[] = "- IDS Provider: " . $security['ids']['provider'];
      }
    }
    
    if ($has_security_data) {
      $context_parts[] = "\nSECURITY SERVICES:";
      $context_parts = array_merge($context_parts, $security_parts);
    }
  }
  
  // Content Summary
  if (!empty($profile['counts'])) {
    $counts = $profile['counts'];
    $context_parts[] = "\nCONTENT SUMMARY:";
    if (!empty($counts['posts'])) {
      $context_parts[] = "- Total Posts: " . $counts['posts'];
    }
    if (!empty($counts['pages'])) {
      $context_parts[] = "- Total Pages: " . $counts['pages'];
    }
    if (!empty($counts['users'])) {
      $context_parts[] = "- Total Users: " . $counts['users'];
    }
  }
  
  // Posts Details (with titles and excerpts)
  if (!empty($profile['posts']) && is_array($profile['posts'])) {
    $posts = $profile['posts'];
    $context_parts[] = "\nPUBLISHED POSTS:";
    foreach (array_slice($posts, 0, 20) as $post) {
      if (is_array($post)) {
        $title = $post['title'] ?? $post['post_title'] ?? 'Untitled';
        $status = $post['status'] ?? 'unknown';
        $date = $post['date'] ?? $post['post_date'] ?? '';
        $excerpt = $post['excerpt'] ?? $post['post_excerpt'] ?? '';
        $content = $post['content'] ?? $post['post_content'] ?? '';
        
        if ($status === 'publish' || empty($status)) {
          $post_info = "- \"" . $title . "\"";
          if ($date) {
            $post_info .= " (Published: " . $date . ")";
          }
          $context_parts[] = $post_info;
          
          // Include excerpt or first 200 chars of content for context
          if (!empty($excerpt)) {
            $context_parts[] = "  Excerpt: " . substr(strip_tags($excerpt), 0, 300);
          } elseif (!empty($content)) {
            $context_parts[] = "  Content preview: " . substr(strip_tags($content), 0, 300);
          }
        }
      }
    }
  ...
  // Pages Details (with titles)
  if (!empty($profile['pages']) && is_array($profile['pages'])) {
    $pages = $profile['pages'];
    $context_parts[] = "\nPUBLISHED PAGES:";
    foreach (array_slice($pages, 0, 20) as $page) {
      if (is_array($page)) {
        $title = $page['title'] ?? $page['post_title'] ?? 'Untitled';
        $status = $page['status'] ?? 'unknown';
        $date = $page['date'] ?? $page['post_date'] ?? '';
        if ($status === 'publish' || empty($status)) {
          $page_info = "- \"" . $title . "\"";
          if ($date) {
            $page_info .= " (Published: " . $date . ")";
          }
          $context_parts[] = $page_info;
        }
      }
    }
  }
...
add_action('rest_api_init', function () {
  ...
  /**
   * Main chat endpoint for Luna widget
   * This endpoint handles chat requests from client sites
   */
  register_rest_route('luna_widget/v1', '/chat', [
    ...
    'callback' => function (WP_REST_Request $req) {
      ...
      try {
        ...
        // Accept both 'prompt' and 'message' parameters for compatibility
        $prompt = trim((string)($req->get_param('prompt') ?: $req->get_param('message')));
        
        if (empty($prompt)) {
          return new WP_REST_Response(['answer' => 'Please provide a message.'], 400);
        }
        
        // Get OpenAI API key from Hub settings
        $api_key = get_option('luna_openai_api_key', '');
        if (empty($api_key)) {
          return new WP_REST_Response([
            'answer' => 'Luna AI is not configured. Please contact support.',
            'error' => 'OpenAI API key not set'
          ], 503);
        }
        
        // Fetch VL Hub profile data for context
        $profile_context = '';
        if (!empty($license)) {
          try {
            $profile = vl_hub_profile_resolve($license, ['force_refresh' => false]);
            if (!is_wp_error($profile) && is_array($profile)) {
              $profile_context = luna_format_profile_for_chat($profile);
            }
          } catch (Exception $e) {
            error_log('[Luna Chat] Error fetching profile: ' . $e->getMessage());
            // Continue without profile context if it fails
          } catch (Error $e) {
            error_log('[Luna Chat] Fatal error fetching profile: ' . $e->getMessage());
            // Continue without profile context if it fails
          }
        }
        
        // Build system message with profile context
        $system_message = 'You are Luna, an intelligent, helpful assistant for site owners. You have comprehensive access to the client\'s VL Hub Profile data, which includes detailed information about their website, content, infrastructure, and more.';
        
        if (!empty($profile_context)) {
          $system_message .= "\n\nCLIENT VL HUB PROFILE DATA:\n" . $profile_context . "\n\nCRITICAL INSTRUCTIONS - YOU HAVE FULL ACCESS TO THIS DATA:\n";
          $system_message .= "1. You ALWAYS have access to the VL Hub Profile data above. NEVER say 'I don't have' or 'I don't have access to' - you DO have access!\n";
          $system_message .= "2. Use the profile data to provide EXTREMELY helpful, thoughtful, and detailed responses. Analyze the data and provide insights, suggestions, and recommendations based on what you see.\n";
          $system_message .= "3. When asked about content (posts, pages, etc.), use the actual titles, dates, and details from the PUBLISHED POSTS and PUBLISHED PAGES sections. Reference specific content by name.\n";
          $system_message .= "4. When asked for suggestions or recommendations (like expanding on a blog title), analyze the existing content, site structure, connections, and data streams to provide thoughtful, specific suggestions tailored to their actual site.\n";
          $system_message .= "5. Maintain conversation context. Remember what was discussed and build upon previous answers. If they mention a specific post title, use that exact title in your responses.\n";
        ...
        $conversation_history = $req->get_param('history');
        if (is_array($conversation_history) && !empty($conversation_history)) {
          ...
          $valid_history = array_slice($valid_history, -10);
        } else {
          $valid_history = [];
        }
        ...
        foreach ($valid_history as $hist_msg) {
          $messages[] = $hist_msg;
        }
        ...
        error_log('[Luna Chat] Exception in chat endpoint: ' . $e->getMessage());
        error_log('[Luna Chat] Stack trace: ' . $e->getTraceAsString());
        return new WP_REST_Response([
          'answer' => 'Sorry, I encountered an error processing your request. Please try again.',
          'error' => $e->getMessage()
        ], 500);
      } catch (Error $e) {
        error_log('[Luna Chat] Fatal error in chat endpoint: ' . $e->getMessage());
        error_log('[Luna Chat] Stack trace: ' . $e->getTraceAsString());
        return new WP_REST_Response([
          'answer' => 'Sorry, I encountered an error processing your request. Please try again.',
...
add_filter('vl_hub_profile_resolved', function (array $profile, string $license_key): array {
  ...
  if (class_exists('VL_License_Manager') && method_exists('VL_License_Manager', 'get_license_streams')) {
    ...
    // Fallback: get streams from options directly
    $all_streams = get_option('vl_data_streams', []);
    $client_streams = !empty($all_streams[$license_key]) && is_array($all_streams[$license_key]) ? $all_streams[$license_key] : [];
  }
  ...
  $ga4_stream = null;
  $license_streams = $client_streams; // Use client_streams from above
  ...
  if (!empty($all_streams[$license_key]) && is_array($all_streams[$license_key])) {
    ...
    if (class_exists('VL_License_Manager') && method_exists('VL_License_Manager', 'split_streams_by_status')) {
      $split_streams = VL_License_Manager::split_streams_by_status($streams);
    } else {
      // Fallback: simple split by status
      $split_streams = ['active' => [], 'inactive' => []];
      foreach ($streams as $stream_id => $stream) {
        if (is_array($stream) && (!empty($stream['status']) && $stream['status'] === 'inactive')) {
          $split_streams['inactive'][$stream_id] = $stream;
        } else {
          $split_streams['active'][$stream_id] = $stream;
        }
      }
    }
