...
}, 9); // Priority 9 to run before default priority 10

// Reorder submenu items to ensure Settings is first and default
add_action('admin_menu', function () {
  global $submenu;
  
  if (!isset($submenu['luna-widget']) || !is_array($submenu['luna-widget']) || empty($submenu['luna-widget'])) {
    return;
  }
  
  // Store original items
  $original_items = $submenu['luna-widget'];
  $settings_item = null;
  $essentials_items = array();
  $other_items = array();
  
  // Categorize all items
  foreach ($original_items as $item) {
    // Skip invalid items but still try to preserve them
    ...
    // Settings item (same slug as parent)
    if ($menu_slug === 'luna-widget') {
      $settings_item = $item;
    }
    // Essentials items
    elseif (strpos($menu_slug, 'luna_essentials') !== false) {
      $essentials_items[] = $item;
    }
    // All other items (Compose, Chat, Training, etc.)
    else {
      $other_items[] = $item;
    }
  }
  
  // Only reorder if we have items and found Settings
  // This ensures we don't break the menu if something unexpected happens
  if ($settings_item !== null) {
    ...
    // 2. Other items (Compose, Chat, Training, etc.) in their original order
    $reordered = array_merge($reordered, $other_items);
    
    // 3. Essentials last
    $reordered = array_merge($reordered, $essentials_items);
    
    // Update the submenu
    $submenu['luna-widget'] = $reordered;
  }
}, 999); // High priority to run after all other plugins

// Redirect parent menu click to Settings page
add_action('admin_init', function () {
  // Only redirect if we're accessing the parent menu directly (not a submenu)
  $current_page = isset($_GET['page']) ? $_GET['page'] : '';
  $post_type = isset($_GET['post_type']) ? $_GET['post_type'] : '';
  $script_name = basename($_SERVER['PHP_SELF']);
  
  // If we're on edit.php with luna_essentials and no other params, redirect to Settings
  // This handles the case where WordPress defaults to the first submenu (Essentials)
  if ($script_name === 'edit.php' && 
      $post_type === 'luna_essentials' && 
      !isset($_GET['action']) && 
      !isset($_GET['post']) &&
      count(array_filter($_GET, function($key) { 
        return !in_array($key, ['post_type', 'page']); 
      }, ARRAY_FILTER_USE_KEY)) === 0) {
    // Check referrer - if it's from the admin menu, redirect to Settings
    $referer = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
    if (strpos($referer, 'admin.php') !== false || empty($referer)) {
      wp_safe_redirect(admin_url('admin.php?page=luna-widget'));
      exit;
    }
  }
});
...
add_action('admin_init', function () {
  ...
  register_setting('luna_widget_settings', LUNA_WIDGET_OPT_SETTINGS, array(
    ...
    'sanitize_callback' => function($a){
      ...
      // Position setting removed - now handled by Supercluster styling
      return array(
...
function luna_widget_collect_wordpress_data() {
  ...
  foreach ($posts_query->posts as $post) {
    ...
    // Prepare content for inclusion (strip HTML tags, keep full content)
    ...
    $posts_data[] = array(
      ...
      'slug' => $post->post_name,
      'content' => $content_plain, // Include full post content (plain text) - CRITICAL for content analysis
      'content_html' => $content, // Include HTML version for reference
  ...
  foreach ($pages_query->posts as $page) {
    ...
    $pages_data[] = array(
      ...
      'slug' => $page->post_name,
      'content' => $content_plain, // Include full page content (plain text) - CRITICAL for content analysis
      'content_html' => $content, // Include HTML version for reference
...
function luna_widget_get_comprehensive_facts() {
  ...
  foreach ($profile_data['connections'] as $conn) {
    // Ensure $conn is an array before accessing its keys
    if (!is_array($conn)) {
      continue;
    }
    if (isset($conn['type']) && $conn['type'] === 'SSL/TLS') {
      $facts['ssl_tls'] = array(
 ...
 */
function luna_openai_messages_with_facts($pid, $user_text, $facts, $is_comprehensive_report = false, $is_composer = false, $use_hub_data = false) {
  ...
  if ($use_hub_data) {
    $facts_text .= "\n🎯 DATA-DRIVEN RESPONSE REQUIRED:\n";
    $facts_text .= "⚠️ You MUST analyze the actual data provided below to identify:\n";
    $facts_text .= "   - Patterns and trends in the client's data\n";
    $facts_text .= "   - Anomalies, discrepancies, or issues that need attention\n";
    $facts_text .= "   - Opportunities for improvement based on real metrics\n";
    $facts_text .= "   - Specific insights derived from the actual numbers and data points\n";
    $facts_text .= "⚠️ DO NOT provide generic responses. Every recommendation must be based on the actual data below.\n";
    $facts_text .= "⚠️ When asked about KPIs, analytics, or metrics, use the EXACT values from the data below.\n";
    $facts_text .= "⚠️ If anomalies or issues are detected in the data, highlight them specifically.\n";
    $facts_text .= "⚠️ NEVER use placeholder text like [Insert X if available] or [N/A]. If data is not available, omit that section entirely.\n\n";
    
    // Add Hub insights if available
    if (isset($facts['hub_insights']) && is_array($facts['hub_insights'])) {
      $insights = $facts['hub_insights'];
      $facts_text .= "\n## VL Hub Profile Insights:\n";
      if (isset($insights['total_streams'])) {
        $facts_text .= "- Total Data Streams: " . (int)$insights['total_streams'] . "\n";
      }
      if (isset($insights['active_streams'])) {
        $facts_text .= "- Active Streams: " . (int)$insights['active_streams'] . "\n";
      }
      if (isset($insights['avg_health_score'])) {
        $facts_text .= "- Average Health Score: " . round((float)$insights['avg_health_score'], 1) . "%\n";
      }
      if (isset($insights['stream_categories']) && is_array($insights['stream_categories'])) {
        $facts_text .= "- Stream Categories: " . implode(', ', array_keys($insights['stream_categories'])) . "\n";
      }
      if (isset($insights['anomalies']) && is_array($insights['anomalies']) && count($insights['anomalies']) > 0) {
        $facts_text .= "\n### Issues Detected:\n";
        foreach (array_slice($insights['anomalies'], 0, 5) as $anomaly) {
          $stream_name = isset($anomaly['stream']) ? $anomaly['stream'] : 'Unknown';
          $issue = isset($anomaly['issue']) ? $anomaly['issue'] : 'Issue detected';
          $facts_text .= "- " . esc_html($stream_name) . ": " . esc_html($issue) . "\n";
        }
      }
      $facts_text .= "\n";
    }
  ...
  if (isset($facts['streams']) && is_array($facts['streams']) && !empty($facts['streams'])) {
    ...
    foreach ($facts['streams'] as $stream) {
      // Ensure $stream is an array before accessing its keys
      if (!is_array($stream)) {
        continue;
      }
      $stream_name = isset($stream['name']) ? $stream['name'] : 'Unnamed Stream';
      $stream_status = isset($stream['status']) ? $stream['status'] : 'unknown';
      $facts_text .= "- " . esc_html($stream_name) . " (" . ucfirst(esc_html($stream_status)) . ")";
      if (isset($stream['health_score']) && is_numeric($stream['health_score']) && $stream['health_score'] > 0) {
        $facts_text .= " - Health: " . number_format((float)$stream['health_score'], 1) . "%";
      }
  ...
  if (isset($facts['connections']) && is_array($facts['connections']) && !empty($facts['connections'])) {
    ...
    foreach ($facts['connections'] as $conn) {
      ...
      $conn_type = isset($conn['type']) ? $conn['type'] : 'Unknown';
      $conn_name = isset($conn['name']) ? $conn['name'] : 'Unnamed';
      $conn_status = isset($conn['status']) ? $conn['status'] : 'unknown';
      $facts_text .= "- " . esc_html($conn_type) . ": " . esc_html($conn_name) . " (" . ucfirst(esc_html($conn_status)) . ")\n";
    }
  }
  ...
  if (isset($facts['wordpress_data']) && is_array($facts['wordpress_data'])) {
    ...
    if (isset($wp_data['posts_data']) && is_array($wp_data['posts_data']) && !empty($wp_data['posts_data'])) {
      ...
      foreach ($wp_data['posts_data'] as $post) {
        ...
        // Include full post content (CRITICAL for content analysis and keyword extraction)
        if (isset($post['content']) && !empty($post['content'])) {
          $post_content = is_string($post['content']) ? $post['content'] : '';
          $content_length = strlen($post_content);
          $facts_text .= "  POST CONTENT (full, " . number_format($content_length) . " characters):\n";
          // Include full content - Luna needs this to analyze keywords, themes, and opportunities
          $facts_text .= "  " . esc_html($post_content) . "\n";
        } elseif (isset($post['excerpt']) && !empty($post['excerpt'])) {
          $excerpt = wp_strip_all_tags($post['excerpt']);
          $facts_text .= "  POST EXCERPT: " . esc_html($excerpt) . "\n";
        }
    ...
    if (isset($wp_data['pages_data']) && is_array($wp_data['pages_data']) && !empty($wp_data['pages_data'])) {
      ...
      foreach ($wp_data['pages_data'] as $page) {
        ...
        // Include full page content (CRITICAL for content analysis)
        if (isset($page['content']) && !empty($page['content'])) {
          $page_content = is_string($page['content']) ? $page['content'] : '';
          $content_length = strlen($page_content);
          $facts_text .= "  PAGE CONTENT (full, " . number_format($content_length) . " characters):\n";
          // Include full content - Luna needs this to analyze keywords, themes, and opportunities
          $facts_text .= "  " . esc_html($page_content) . "\n";
        } elseif (isset($page['excerpt']) && !empty($page['excerpt'])) {
          $excerpt = wp_strip_all_tags($page['excerpt']);
          $facts_text .= "  PAGE EXCERPT: " . esc_html($excerpt) . "\n";
        }
    ...
    // Content Metrics - Calculate if not provided
    $facts_text .= "\nCONTENT METRICS:\n";
    $total_word_count = 0;
    $posts_with_word_count = 0;
    $keyword_counts = array();
    
    // Calculate total word count from posts if not in metrics
    if (isset($wp_data['posts_data']) && is_array($wp_data['posts_data']) && !empty($wp_data['posts_data'])) {
      foreach ($wp_data['posts_data'] as $post) {
        if (isset($post['word_count']) && is_numeric($post['word_count'])) {
          $total_word_count += (int)$post['word_count'];
        ...
        if (isset($post['categories']) && is_array($post['categories'])) {
          foreach ($post['categories'] as $cat) {
            if (!empty($cat)) {
              $keyword_counts[strtolower(trim($cat))] = isset($keyword_counts[strtolower(trim($cat))]) ? $keyword_counts[strtolower(trim($cat))] + 1 : 1;
            }
          }
        }
        if (isset($post['tags']) && is_array($post['tags'])) {
          foreach ($post['tags'] as $tag) {
            if (!empty($tag)) {
              $keyword_counts[strtolower(trim($tag))] = isset($keyword_counts[strtolower(trim($tag))]) ? $keyword_counts[strtolower(trim($tag))] + 1 : 1;
            }
          }
        }
      }
    ...
    // Use provided metrics or calculated values
    if (isset($wp_data['content_metrics']) && is_array($wp_data['content_metrics'])) {
      $metrics = $wp_data['content_metrics'];
      if (isset($metrics['total_word_count']) && (int)$metrics['total_word_count'] > 0) {
      ...
      if (isset($metrics['top_keywords']) && is_array($metrics['top_keywords']) && !empty($metrics['top_keywords'])) {
        $keyword_counts = $metrics['top_keywords'];
      }
    }
    
    // Display calculated/provided metrics
    if ($total_word_count > 0) {
      $facts_text .= "- Total Word Count: " . number_format($total_word_count) . "\n";
    }
    if ($posts_with_word_count > 0) {
      $avg_words = round($total_word_count / $posts_with_word_count);
      $facts_text .= "- Average Words Per Post: " . number_format($avg_words) . "\n";
    }
    if (!empty($keyword_counts)) {
      ...
      $keyword_list = array();
      $count = 0;
      foreach ($keyword_counts as $keyword => $usage_count) {
  ...
  // Add Analytics Data (GA4, Search Console, Lighthouse, Competitor) - CRITICAL for analytics questions
  if (isset($facts['analytics']) && is_array($facts['analytics']) && !empty($facts['analytics'])) {
    $analytics = $facts['analytics'];
    $facts_text .= "\n=== ANALYTICS & PERFORMANCE DATA ===\n";
    $facts_text .= "⚠️ CRITICAL: This section contains REAL analytics data from Google Analytics 4, Google Search Console, Lighthouse reports, and competitor analyses.\n";
    $facts_text .= "⚠️ When asked about web traffic, analytics, performance, SEO, or advertising effectiveness, you MUST use the EXACT data provided below.\n";
    $facts_text .= "⚠️ DO NOT say you don't have access to analytics data - you have comprehensive data below.\n\n";
    
    // Google Analytics 4 Data
    if (isset($analytics['ga4']) && is_array($analytics['ga4']) && !empty($analytics['ga4'])) {
      $ga4 = $analytics['ga4'];
      $facts_text .= "GOOGLE ANALYTICS 4 DATA:\n";
      $facts_text .= "Property: " . (isset($ga4['name']) ? esc_html($ga4['name']) : 'GA4 Property') . "\n";
      if (isset($ga4['property_id'])) {
        $facts_text .= "Property ID: " . esc_html($ga4['property_id']) . "\n";
      }
      if (isset($ga4['last_synced'])) {
        $facts_text .= "Last Synced: " . esc_html($ga4['last_synced']) . "\n";
      }
      if (isset($ga4['date_range']) && is_array($ga4['date_range'])) {
        $facts_text .= "Date Range: " . (isset($ga4['date_range']['startDate']) ? esc_html($ga4['date_range']['startDate']) : 'N/A') . " to " . (isset($ga4['date_range']['endDate']) ? esc_html($ga4['date_range']['endDate']) : 'N/A') . "\n";
      }
      
      if (isset($ga4['metrics']) && is_array($ga4['metrics'])) {
        $metrics = $ga4['metrics'];
        $facts_text .= "\nTRAFFIC METRICS:\n";
        if (isset($metrics['totalUsers'])) $facts_text .= "- Total Users: " . number_format((int)$metrics['totalUsers']) . "\n";
        if (isset($metrics['newUsers'])) $facts_text .= "- New Users: " . number_format((int)$metrics['newUsers']) . "\n";
        if (isset($metrics['activeUsers'])) $facts_text .= "- Active Users: " . number_format((int)$metrics['activeUsers']) . "\n";
        if (isset($metrics['sessions'])) $facts_text .= "- Sessions: " . number_format((int)$metrics['sessions']) . "\n";
        if (isset($metrics['screenPageViews'])) $facts_text .= "- Page Views: " . number_format((int)$metrics['screenPageViews']) . "\n";
        if (isset($metrics['bounceRate'])) $facts_text .= "- Bounce Rate: " . number_format((float)$metrics['bounceRate'] * 100, 2) . "%\n";
        if (isset($metrics['averageSessionDuration'])) $facts_text .= "- Avg Session Duration: " . number_format((float)$metrics['averageSessionDuration'], 0) . " seconds\n";
        if (isset($metrics['engagementRate'])) $facts_text .= "- Engagement Rate: " . number_format((float)$metrics['engagementRate'] * 100, 2) . "%\n";
        if (isset($metrics['engagedSessions'])) $facts_text .= "- Engaged Sessions: " . number_format((int)$metrics['engagedSessions']) . "\n";
        if (isset($metrics['eventCount'])) $facts_text .= "- Total Events: " . number_format((int)$metrics['eventCount']) . "\n";
        if (isset($metrics['conversions'])) $facts_text .= "- Conversions: " . number_format((int)$metrics['conversions']) . "\n";
        if (isset($metrics['totalRevenue'])) $facts_text .= "- Total Revenue: $" . number_format((float)$metrics['totalRevenue'], 2) . "\n";
      }
      
      // Geographic Data
      if (isset($ga4['dimensions']['geographic']) && is_array($ga4['dimensions']['geographic']) && !empty($ga4['dimensions']['geographic'])) {
        $facts_text .= "\nTOP GEOGRAPHIC LOCATIONS:\n";
        $geo_count = 0;
        foreach ($ga4['dimensions']['geographic'] as $geo) {
          if ($geo_count >= 10) break;
          if (isset($geo['dimensionValues']) && is_array($geo['dimensionValues']) && count($geo['dimensionValues']) >= 3) {
            $country = $geo['dimensionValues'][0]['value'] ?? 'Unknown';
            $region = $geo['dimensionValues'][1]['value'] ?? 'Unknown';
            $city = $geo['dimensionValues'][2]['value'] ?? 'Unknown';
            $users = isset($geo['metricValues'][0]['value']) ? number_format((int)$geo['metricValues'][0]['value']) : '0';
            $sessions = isset($geo['metricValues'][2]['value']) ? number_format((int)$geo['metricValues'][2]['value']) : '0';
            $facts_text .= "- " . esc_html($city) . ", " . esc_html($region) . ", " . esc_html($country) . ": " . $users . " users, " . $sessions . " sessions\n";
            $geo_count++;
          }
        }
      }
      ...
      // Device Data
      if (isset($ga4['dimensions']['device']) && is_array($ga4['dimensions']['device']) && !empty($ga4['dimensions']['device'])) {
        $facts_text .= "\nDEVICE BREAKDOWN:\n";
        foreach ($ga4['dimensions']['device'] as $device) {
          if (isset($device['dimensionValues']) && is_array($device['dimensionValues']) && count($device['dimensionValues']) >= 3) {
            $device_type = $device['dimensionValues'][0]['value'] ?? 'Unknown';
            $device_brand = $device['dimensionValues'][1]['value'] ?? 'Unknown';
            $device_browser = $device['dimensionValues'][2]['value'] ?? 'Unknown';
            $users = isset($device['metricValues'][0]['value']) ? number_format((int)$device['metricValues'][0]['value']) : '0';
            $sessions = isset($device['metricValues'][2]['value']) ? number_format((int)$device['metricValues'][2]['value']) : '0';
            $facts_text .= "- " . esc_html($device_type) . " (" . esc_html($device_brand) . " " . esc_html($device_browser) . "): " . $users . " users, " . $sessions . " sessions\n";
          }
        }
      }
      
      // Traffic Sources
      if (isset($ga4['dimensions']['traffic']) && is_array($ga4['dimensions']['traffic']) && !empty($ga4['dimensions']['traffic'])) {
        $facts_text .= "\nTRAFFIC SOURCES:\n";
        foreach ($ga4['dimensions']['traffic'] as $traffic) {
          if (isset($traffic['dimensionValues']) && is_array($traffic['dimensionValues']) && count($traffic['dimensionValues']) >= 3) {
            $source = $traffic['dimensionValues'][0]['value'] ?? 'Unknown';
            $medium = $traffic['dimensionValues'][1]['value'] ?? 'Unknown';
            $campaign = $traffic['dimensionValues'][2]['value'] ?? 'Unknown';
            $users = isset($traffic['metricValues'][0]['value']) ? number_format((int)$traffic['metricValues'][0]['value']) : '0';
            $sessions = isset($traffic['metricValues'][2]['value']) ? number_format((int)$traffic['metricValues'][2]['value']) : '0';
            $pageviews = isset($traffic['metricValues'][3]['value']) ? number_format((int)$traffic['metricValues'][3]['value']) : '0';
            $facts_text .= "- " . esc_html($source) . " / " . esc_html($medium) . " / " . esc_html($campaign) . ": " . $users . " users, " . $sessions . " sessions, " . $pageviews . " pageviews\n";
          }
        }
      }
      
      // Top Pages
      if (isset($ga4['dimensions']['pages']) && is_array($ga4['dimensions']['pages']) && !empty($ga4['dimensions']['pages'])) {
        $facts_text .= "\nTOP PAGES:\n";
        $page_count = 0;
        foreach ($ga4['dimensions']['pages'] as $page) {
          if ($page_count >= 15) break;
          if (isset($page['dimensionValues']) && is_array($page['dimensionValues']) && count($page['dimensionValues']) >= 2) {
            $page_path = $page['dimensionValues'][0]['value'] ?? 'Unknown';
            $page_title = $page['dimensionValues'][1]['value'] ?? 'Unknown';
            $users = isset($page['metricValues'][0]['value']) ? number_format((int)$page['metricValues'][0]['value']) : '0';
            $sessions = isset($page['metricValues'][1]['value']) ? number_format((int)$page['metricValues'][1]['value']) : '0';
            $pageviews = isset($page['metricValues'][2]['value']) ? number_format((int)$page['metricValues'][2]['value']) : '0';
            $facts_text .= "- " . esc_html($page_title) . " (" . esc_html($page_path) . "): " . $users . " users, " . $sessions . " sessions, " . $pageviews . " pageviews\n";
            $page_count++;
          }
        }
      }
      ...
      // Events
      if (isset($ga4['dimensions']['events']) && is_array($ga4['dimensions']['events']) && !empty($ga4['dimensions']['events'])) {
        $facts_text .= "\nTOP EVENTS:\n";
        foreach ($ga4['dimensions']['events'] as $event) {
          if (isset($event['dimensionValues']) && is_array($event['dimensionValues']) && count($event['dimensionValues']) >= 1) {
            $event_name = $event['dimensionValues'][0]['value'] ?? 'Unknown';
            $event_count = isset($event['metricValues'][0]['value']) ? number_format((int)$event['metricValues'][0]['value']) : '0';
            $facts_text .= "- " . esc_html($event_name) . ": " . $event_count . " events\n";
          }
        }
      }
    ...
    if (isset($analytics['search_console']) && is_array($analytics['search_console']) && !empty($analytics['search_console'])) {
      $gsc = $analytics['search_console'];
      $facts_text .= "GOOGLE SEARCH CONSOLE DATA:\n";
      if (isset($gsc['last_sync'])) {
        $facts_text .= "Last Sync: " . esc_html($gsc['last_sync']) . "\n";
      }
      if (isset($gsc['data_points'])) {
        $facts_text .= "Data Points: " . number_format((int)$gsc['data_points']) . "\n";
      }
      
      // Search Queries
      if (isset($gsc['search_queries']) && is_array($gsc['search_queries']) && !empty($gsc['search_queries'])) {
        $facts_text .= "\nTOP SEARCH QUERIES:\n";
        $query_count = 0;
        foreach ($gsc['search_queries'] as $query) {
          if ($query_count >= 10) break;
          if (isset($query['keys']) && is_array($query['keys']) && !empty($query['keys'])) {
            $query_text = $query['keys'][0] ?? 'Unknown';
            $clicks = isset($query['clicks']) ? number_format((int)$query['clicks']) : '0';
            $impressions = isset($query['impressions']) ? number_format((int)$query['impressions']) : '0';
            $ctr = isset($query['ctr']) ? number_format((float)$query['ctr'] * 100, 2) : '0.00';
            $position = isset($query['position']) ? number_format((float)$query['position'], 1) : 'N/A';
            $facts_text .= "- \"" . esc_html($query_text) . "\": " . $clicks . " clicks, " . $impressions . " impressions, " . $ctr . "% CTR, Position " . $position . "\n";
            $query_count++;
          }
        }
      }
      ...
      // Top Pages
      if (isset($gsc['top_pages']) && is_array($gsc['top_pages']) && !empty($gsc['top_pages'])) {
        $facts_text .= "\nTOP PAGES (SEARCH PERFORMANCE):\n";
        $page_count = 0;
        foreach ($gsc['top_pages'] as $page) {
          if ($page_count >= 10) break;
          if (isset($page['keys']) && is_array($page['keys']) && !empty($page['keys'])) {
            $page_url = $page['keys'][0] ?? 'Unknown';
            $clicks = isset($page['clicks']) ? number_format((int)$page['clicks']) : '0';
            $impressions = isset($page['impressions']) ? number_format((int)$page['impressions']) : '0';
            $ctr = isset($page['ctr']) ? number_format((float)$page['ctr'] * 100, 2) : '0.00';
            $position = isset($page['position']) ? number_format((float)$page['position'], 1) : 'N/A';
            $facts_text .= "- " . esc_html($page_url) . ": " . $clicks . " clicks, " . $impressions . " impressions, " . $ctr . "% CTR, Position " . $position . "\n";
            $page_count++;
          }
        }
      }
      ...
      // Countries
      if (isset($gsc['countries']) && is_array($gsc['countries']) && !empty($gsc['countries'])) {
        $facts_text .= "\nSEARCH PERFORMANCE BY COUNTRY:\n";
        foreach ($gsc['countries'] as $country) {
          if (isset($country['keys']) && is_array($country['keys']) && !empty($country['keys'])) {
            $country_code = $country['keys'][0] ?? 'Unknown';
            $clicks = isset($country['clicks']) ? number_format((int)$country['clicks']) : '0';
            $impressions = isset($country['impressions']) ? number_format((int)$country['impressions']) : '0';
            $position = isset($country['position']) ? number_format((float)$country['position'], 1) : 'N/A';
            $facts_text .= "- " . esc_html($country_code) . ": " . $clicks . " clicks, " . $impressions . " impressions, Position " . $position . "\n";
          }
        }
      }
    ...
    // Lighthouse Reports
    if (isset($analytics['lighthouse']) && is_array($analytics['lighthouse']) && !empty($analytics['lighthouse'])) {
      $facts_text .= "LIGHTHOUSE PERFORMANCE REPORTS:\n";
      foreach ($analytics['lighthouse'] as $report) {
        if (isset($report['scores']) && is_array($report['scores'])) {
          $facts_text .= "\nReport for: " . (isset($report['url']) ? esc_html($report['url']) : 'N/A') . "\n";
          $facts_text .= "Date: " . (isset($report['date']) ? esc_html($report['date']) : 'N/A') . "\n";
          $facts_text .= "Performance: " . (isset($report['scores']['performance']) ? number_format((int)$report['scores']['performance']) : 'N/A') . "/100\n";
          $facts_text .= "Accessibility: " . (isset($report['scores']['accessibility']) ? number_format((int)$report['scores']['accessibility']) : 'N/A') . "/100\n";
          $facts_text .= "Best Practices: " . (isset($report['scores']['best_practices']) ? number_format((int)$report['scores']['best_practices']) : 'N/A') . "/100\n";
          $facts_text .= "SEO: " . (isset($report['scores']['seo']) ? number_format((int)$report['scores']['seo']) : 'N/A') . "/100\n";
          $facts_text .= "Overall: " . (isset($report['scores']['overall']) ? number_format((int)$report['scores']['overall']) : 'N/A') . "/100\n";
          
          if (isset($report['opportunities']) && is_array($report['opportunities']) && !empty($report['opportunities'])) {
            $facts_text .= "Key Opportunities:\n";
            $opp_count = 0;
            foreach ($report['opportunities'] as $opp) {
              if ($opp_count >= 5) break;
              $title = isset($opp['title']) ? esc_html($opp['title']) : 'Opportunity';
              $savings = isset($opp['savings']) ? esc_html($opp['savings']) : '';
              $facts_text .= "- " . $title . ($savings ? " (Potential savings: " . $savings . ")" : "") . "\n";
              $opp_count++;
            }
          }
        }
    ...
    // Competitor Analysis
    if (isset($analytics['competitor']) && is_array($analytics['competitor']) && !empty($analytics['competitor'])) {
      $facts_text .= "COMPETITOR ANALYSIS:\n";
      foreach ($analytics['competitor'] as $competitor) {
        $facts_text .= "\nCompetitor: " . (isset($competitor['name']) ? esc_html($competitor['name']) : 'Unknown') . "\n";
        $facts_text .= "URL: " . (isset($competitor['url']) ? esc_html($competitor['url']) : 'N/A') . "\n";
        if (isset($competitor['domain_ranking'])) {
          $facts_text .= "Domain Ranking: " . number_format((float)$competitor['domain_ranking'], 2) . "\n";
        }
        if (isset($competitor['lighthouse']) && is_array($competitor['lighthouse'])) {
          $lh = $competitor['lighthouse'];
          $facts_text .= "Lighthouse Scores - Performance: " . (isset($lh['performance']) ? number_format((int)$lh['performance']) : 'N/A') . ", SEO: " . (isset($lh['seo']) ? number_format((int)$lh['seo']) : 'N/A') . "\n";
        }
        if (isset($competitor['public_pages'])) {
          $facts_text .= "Public Pages: " . number_format((int)$competitor['public_pages']) . "\n";
        }
      }
      $facts_text .= "\n";
    }
    
    $facts_text .= "⚠️ END OF ANALYTICS DATA ===\n";
    $facts_text .= "⚠️ When answering questions about web traffic, analytics, performance, SEO, or advertising:\n";
    $facts_text .= "   - Use the EXACT numbers from the GA4 metrics above\n";
    $facts_text .= "   - Reference specific pages, traffic sources, and geographic data\n";
    $facts_text .= "   - Identify spikes/dips by comparing session and pageview data\n";
    $facts_text .= "   - Correlate Search Console data with GA4 traffic sources\n";
    $facts_text .= "   - Use Lighthouse scores to discuss performance opportunities\n";
    $facts_text .= "   - NEVER say you don't have access to analytics - you have comprehensive data above.\n\n";
  }
...
- You have REAL-TIME ANALYTICS DATA from Google Analytics 4, Google Search Console, Lighthouse, and Competitor Analysis:
  * GA4 Metrics: Total users, new users, sessions, page views, bounce rate, engagement rate, conversions, revenue
  * GA4 Dimensions: Geographic data (cities, countries), device breakdown (desktop/mobile), traffic sources, top pages, events
  * Search Console: Search queries with impressions/clicks/CTR/position, top pages performance, country breakdowns
  * Lighthouse: Performance, accessibility, SEO, and best practices scores with specific opportunities
  * Competitor: Domain rankings, Lighthouse scores, keyword analysis
  * When asked about web traffic, analytics, performance, SEO, or advertising effectiveness, you MUST use the EXACT data from the Analytics section
  * NEVER say you don't have access to analytics data - you have comprehensive, real-time data available
  * Identify traffic spikes/dips by comparing session and pageview metrics across time periods
  * Correlate Google Ad spend effectiveness with traffic source data and conversion metrics
  * Provide specific insights based on geographic, device, and traffic source breakdowns
...
";
    ...
    // Add data-driven response instructions when Hub data is available
    if ($use_hub_data) {
      $default_composer_prompt .= "

🎯 DATA-DRIVEN RESPONSE REQUIREMENTS (VL Hub Profile Data Available):

- You MUST analyze the actual data provided in the facts section to identify:
  * Patterns and trends in the client's actual metrics and data streams
  * Anomalies, discrepancies, or issues that need attention (e.g., low health scores, errors, warnings)
  * Opportunities for improvement based on REAL numbers and data points
  * Specific insights derived from the actual values, not generic statements

- When asked about KPIs, analytics, metrics, or performance:
  * Use the EXACT values from the data provided (e.g., \"Your site has 127 published posts\" not \"your site has posts\")
  * Reference specific data streams, their health scores, and status
  * Identify patterns: \"I notice 3 of your data streams have health scores below 70%, which suggests...\"
  * Point out anomalies: \"Your WordPress site shows 5 plugins needing updates, which could impact security...\"

- When providing recommendations:
  * Base EVERY recommendation on the actual data patterns you observe
  * If you see low health scores, recommend addressing those specific issues
  * If you see update alerts, recommend updating those specific plugins/themes
  * If you see traffic patterns, reference the actual sources and volumes

- NEVER provide generic responses like \"Here's a comprehensive rundown\" without referencing actual data
- ALWAYS include specific numbers, metrics, and data points from the facts section
- ALWAYS identify at least one pattern, anomaly, or discrepancy that the client should know about
- ALWAYS make recommendations actionable and specific to their actual data

🚫 ABSOLUTE PROHIBITION ON PLACEHOLDERS:

- You MUST NEVER use placeholder text like:
  * [Insert Total Word Count if available]
  * [Insert Average Word Count if available]
  * [Insert Top Keywords if available]
  * [N/A]
  * [Unknown]
  * Any text in square brackets [like this]
  * Any text in parentheses indicating missing data (e.g., \"(if available)\", \"(if known)\")

- If a metric or data point is not available in the facts section:
  * DO NOT mention it at all - simply omit that section
  * DO NOT create templates with empty fields
  * DO NOT use placeholders or brackets to indicate missing data

- CORRECT approach when data is missing:
  * If word count is not available, do not mention \"Total Word Count\" at all
  * If keywords are not available, do not mention \"Top Keywords\" at all
  * Only include metrics that are actually present in the facts section

- Examples of FORBIDDEN placeholder usage:
  * \"Total Word Count: [Insert Total Word Count if available]\" ❌
  * \"Average Word Count: [Insert Average Word Count if available]\" ❌
  * \"Top Keywords: [Insert Top Keywords if available]\" ❌
  * \"Engagement Rate: [N/A]\" ❌

- Examples of CORRECT usage:
  * If word count is available: \"Total Word Count: 45,230 words\" ✅
  * If word count is NOT available: Simply omit the line entirely ✅
  * If keywords are available: \"Top Keywords: technology (15), business (12), cloud (8)\" ✅
  * If keywords are NOT available: Simply omit the line entirely ✅

CRITICAL INSTRUCTION FOR ALL RESPONSES:

- Use EXACT numbers from the facts section - never estimate or round unless the number is already rounded in the data
- If a metric is not in the facts section, DO NOT include it in your response - omit it completely
- Never create templates with empty fields or placeholders
- Every number, metric, and data point must come directly from the facts section
- If you cannot find specific data, do not mention it - focus on what IS available

Example of GOOD response:
\"Based on your VL Hub Profile data, I can see you have 12 active data streams with an average health score of 78.5%. However, I notice 3 streams (Security Monitor, Content Management, and Analytics) have health scores below 70%, which indicates potential issues. Your WordPress site shows 127 published posts, 45 pages, and 23 active plugins. I've identified that 5 plugins need updates, which could impact security. Here are specific recommendations based on these findings...\"

Example of BAD response (generic, not data-driven):
\"Here's a comprehensive rundown of your core Site Analytics and Digital Marketing KPIs. This overview will help you understand performance metrics...\" (This is generic and doesn't reference actual data)

Example of BAD response (with placeholders):
\"Total Word Count: [Insert Total Word Count if available]
Average Word Count: [Insert Average Word Count if available]
Top Keywords: [Insert Top Keywords if available]\" ❌

Example of GOOD response (when data is missing):
\"Your WordPress site shows 127 published posts, 45 pages, and 23 active plugins. I've identified that 5 plugins need updates...\" (Omits unavailable metrics entirely) ✅
";
    }
...
/* ============================================================
 * GLOBAL CORS HANDLER - Handle OPTIONS preflight early
 * ============================================================ */
add_action('init', function() {
  // Handle OPTIONS preflight requests BEFORE WordPress processes them
  if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS' && 
      (strpos($_SERVER['REQUEST_URI'], '/wp-json/luna_widget/v1/') !== false ||
       strpos($_SERVER['REQUEST_URI'], 'luna_widget/v1/') !== false)) {
    ...
    header('Access-Control-Allow-Headers: Content-Type, Authorization, X-Requested-With, Accept, X-WP-Nonce, X-Luna-Composer, X-Luna-License, x-luna-composer, x-luna-license, X-LUNA-COMPOSER, X-LUNA-LICENSE');
...
/* ============================================================
 * GLOBAL CORS HANDLER FOR REST API
 * ============================================================ */
add_action('rest_api_init', function() {
  // Handle CORS headers for all luna_widget endpoints
  add_filter('rest_pre_serve_request', function($served, $result, $request, $server) {
    $route = $request->get_route();
    if (strpos($route, '/luna_widget/v1/') === 0) {
      $origin = isset($_SERVER['HTTP_ORIGIN']) ? $_SERVER['HTTP_ORIGIN'] : '';
      $allowed = array('https://supercluster.visiblelight.ai', 'https://visiblelight.ai');
 ...
 * ============================================================ */
add_action('rest_api_init', function () {
  // Chat endpoint - must accept both POST and OPTIONS for CORS
  ...
  // Composer create WordPress post endpoint
  register_rest_route('luna_widget/v1', '/composer/create-post', array(
    'methods' => 'POST',
    'callback' => 'luna_widget_composer_create_post',
    'permission_callback' => '__return_true',
  ));
...
/**
 * Create WordPress post from Luna Composer content
 */
function luna_widget_composer_create_post(WP_REST_Request $req) {
  $license = $req->get_param('license');
  $document_id = $req->get_param('document_id');
  ...
  if (empty($title)) {
    $title = 'New Post from Luna Composer';
  }
  ...
  // Get client site URL from license
  // Check if we're on the Hub (visiblelight.ai) and need to post to client site
  $client_site_url = null;
  $is_hub = (strpos(home_url(), 'visiblelight.ai') !== false);
  
  if ($is_hub) {
    // Try to get client site URL from Hub's license manager
    if (class_exists('VL_License_Manager')) {
      $license_record = VL_License_Manager::lic_lookup_by_key($license);
      if ($license_record && isset($license_record['site']) && !empty($license_record['site'])) {
        $client_site_url = $license_record['site'];
      }
    }
    
    // If we have a client site URL, try to post to the client's WordPress site
    // But if it fails, fall back to creating on the Hub
    if (!empty($client_site_url)) {
      // Make request to client site's Luna Widget endpoint (if available) or WordPress REST API
      $client_endpoints = array(
        trailingslashit($client_site_url) . 'wp-json/luna_widget/v1/composer/create-post',
        trailingslashit($client_site_url) . 'wp-json/wp/v2/posts',
      );
      ...
      $post_data = array(
        ...
        'status' => in_array($status, array('draft', 'publish', 'pending', 'private')) ? $status : 'draft',
      );
      
      $last_error = null;
      $last_code = 0;
      $client_site_reachable = false;
      
      // First, quickly test if client site is reachable
      $test_response = wp_remote_get(trailingslashit($client_site_url), array(
        'timeout' => 5,
        'sslverify' => false, // Disable SSL verification for client sites
        'redirection' => 0,
      ));
      
      if (!is_wp_error($test_response)) {
        $client_site_reachable = true;
      }
      
      // Only try to post to client site if it's reachable
      if ($client_site_reachable) {
        foreach ($client_endpoints as $client_endpoint) {
          $response = wp_remote_post($client_endpoint, array(
            ...
            'sslverify' => false, // Disable SSL verification for client sites (many have self-signed certs)
          ...
          if (is_wp_error($response)) {
            ...
            // If it's a timeout or connection error, skip to next endpoint or fallback
            if ($error_code === 'http_request_failed' || strpos($error_message, 'timeout') !== false || strpos($error_message, 'Operation timed out') !== false) {
              $last_error = 'Client site unreachable or timeout';
              continue;
            }
          ...
          if ($response_code === 200 || $response_code === 201) {
            $post_id = isset($response_data['post_id']) ? $response_data['post_id'] : (isset($response_data['id']) ? $response_data['id'] : null);
            $edit_url = isset($response_data['edit_url']) ? $response_data['edit_url'] : (isset($response_data['link']) ? $response_data['link'] : (trailingslashit($client_site_url) . 'wp-admin/post.php?action=edit&post=' . $post_id));
            
            return rest_ensure_response(array(
              'ok' => true,
              'message' => 'WordPress post created successfully on client site.',
              'post_id' => $post_id,
              'edit_url' => $edit_url,
              'site_url' => $client_site_url
            ));
          } else {
            $last_error = isset($response_data['message']) ? $response_data['message'] : 'HTTP ' . $response_code;
          }
        }
      } else {
        $last_error = 'Client site unreachable';
      }
      
      // If client site posting failed, fall back to creating on Hub
      // This ensures the post is created even if client site is down
      error_log('[Luna Widget] Failed to create post on client site (' . $client_site_url . '): ' . $last_error . '. Falling back to Hub.');
    }
  }
  
  // For client sites or if no client site URL found, create the post directly on the current site (Hub)
  $current_site_url = home_url();
  
  // Check for duplicate posts created in the last 5 minutes with the same title
  // This prevents rapid-fire duplicate post creation
  global $wpdb;
  $sanitized_title = sanitize_text_field($title);
  $recent_duplicate_ids = $wpdb->get_col($wpdb->prepare(
  ...
  if (!empty($recent_duplicate_ids)) {
    foreach ($recent_duplicate_ids as $post_id) {
      $post = get_post($post_id);
      if ($post) {
        $recent_duplicates[] = $post;
      }
    }
  }
  
  // Also check by document_id meta if available
  if ($document_id) {
    $existing_by_doc = get_posts(array(
      'post_type' => 'post',
      'post_status' => 'any',
      'meta_query' => array(
        array(
          'key' => 'luna_composer_document_id',
          'value' => $document_id,
          'compare' => '=',
        ),
      ),
    ...
    if (!empty($existing_by_doc)) {
      $existing_post = $existing_by_doc[0];
      $edit_url = admin_url('post.php?action=edit&post=' . $existing_post->ID);
      return rest_ensure_response(array(
        'ok' => true,
        'message' => 'Post already exists (created recently).',
  ...
  if (!empty($recent_duplicates)) {
    $existing_post = $recent_duplicates[0];
    $edit_url = admin_url('post.php?action=edit&post=' . $existing_post->ID);
    return rest_ensure_response(array(
      'ok' => true,
      'message' => 'Post with this title already exists (created recently).',
      'post_id' => $existing_post->ID,
      'edit_url' => $edit_url,
      'site_url' => $current_site_url,
      'duplicate' => true,
  ...
  // Create WordPress post on current site (Hub)
  $post_data = array(
    'post_title' => sanitize_text_field($title),
    'post_content' => wp_kses_post($content),
    'post_status' => in_array($status, array('draft', 'publish', 'pending', 'private')) ? $status : 'draft',
    'post_type' => 'post',
  ...
  if (is_wp_error($post_id)) {
    return new WP_Error('create_failed', 'Failed to create WordPress post: ' . $post_id->get_error_message(), array('status' => 500));
  }
  
  // Save document_id and license as meta for reference
  if ($document_id) {
  ...
  // Store client site URL if available for future reference
  if (!empty($client_site_url)) {
    update_post_meta($post_id, 'luna_composer_client_site', $client_site_url);
  }
  
  $edit_url = admin_url('post.php?action=edit&post=' . $post_id);
  
  $message = 'WordPress post created successfully on Hub.';
  if (!empty($client_site_url)) {
    $message .= ' (Client site was unreachable, post created on Hub instead)';
  }
  ...
  return rest_ensure_response(array(
    ...
    'site_url' => $current_site_url,
    'created_on_hub' => true,
    'client_site_url' => !empty($client_site_url) ? $client_site_url : null
  ));
}
...
function luna_widget_chat_handler(WP_REST_Request $req) {
  ...
  if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    ...
    // Include ALL possible header variations (browsers normalize to lowercase for CORS)
    $response->header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, Accept, X-WP-Nonce, X-Luna-Composer, X-Luna-License, x-luna-composer, x-luna-license, X-LUNA-COMPOSER, X-LUNA-LICENSE');
    $response->header('Access-Control-Max-Age', '86400');
    
    // Send headers immediately
    status_header(200);
    header('Access-Control-Allow-Origin: ' . (!empty($origin) && in_array($origin, $allowed) ? $origin : '*'));
    header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
    header('Access-Control-Allow-Headers: Content-Type, Authorization, X-Requested-With, Accept, X-WP-Nonce, X-Luna-Composer, X-Luna-License, x-luna-composer, x-luna-license, X-LUNA-COMPOSER, X-LUNA-LICENSE');
    header('Access-Control-Allow-Credentials: true');
    header('Access-Control-Max-Age: 86400');
  ...
  try {
    ...
    // Get client data if provided (from Luna Composer)
    $client_data = $req->get_param('client_data');
    $use_hub_data = $req->get_param('use_hub_data');
    
    // Debug: Log what we received
    error_log('[Luna Widget] Received request. use_hub_data: ' . ($use_hub_data ? 'yes' : 'no') . ', has_client_data: ' . ($client_data ? 'yes' : 'no'));
    if ($client_data && is_array($client_data)) {
        error_log('[Luna Widget] client_data keys: ' . implode(', ', array_keys($client_data)));
        if (isset($client_data['analytics'])) {
            error_log('[Luna Widget] Analytics data present: ' . (is_array($client_data['analytics']) ? 'yes (array)' : 'no'));
            if (is_array($client_data['analytics'])) {
                error_log('[Luna Widget] Analytics keys: ' . implode(', ', array_keys($client_data['analytics'])));
                error_log('[Luna Widget] Has GA4: ' . (isset($client_data['analytics']['ga4']) ? 'yes' : 'no'));
                error_log('[Luna Widget] Has Search Console: ' . (isset($client_data['analytics']['search_console']) ? 'yes' : 'no'));
            }
        }
    }
    
    // If client data is provided, merge it directly into facts array (prioritize client data as it's more recent)
    // CRITICAL: Always merge client_data if provided, even if use_hub_data is false (for analytics data)
    if ($client_data && is_array($client_data)) {
      // Merge WordPress data directly into facts array - CLIENT DATA TAKES PRIORITY
      if (isset($client_data['wordpress_data']) && is_array($client_data['wordpress_data']) && !empty($client_data['wordpress_data'])) {
        // Client data is more recent and comprehensive - use it as primary source
        $facts['wordpress_data'] = $client_data['wordpress_data'];
        error_log('[Luna Widget] Using client_data WordPress data. Has posts_total: ' . (isset($client_data['wordpress_data']['posts_total']) ? $client_data['wordpress_data']['posts_total'] : 'no'));
      }
      
      // Merge streams data - CLIENT DATA TAKES PRIORITY
      if (isset($client_data['streams']) && is_array($client_data['streams']) && !empty($client_data['streams'])) {
        // Convert associative array to indexed array if needed
        $streams_array = array();
        foreach ($client_data['streams'] as $stream_id => $stream) {
          if (is_array($stream)) {
            if (!isset($stream['id'])) {
              $stream['id'] = $stream_id;
            }
        ...
        if (!empty($streams_array)) {
          $facts['streams'] = $streams_array;
        }
      }
      
      // Merge connections data
      if (isset($client_data['connections']) && is_array($client_data['connections']) && !empty($client_data['connections'])) {
        $facts['connections'] = $client_data['connections'];
      }
      
      // Store insights for later use in facts text
      if (isset($client_data['insights']) && is_array($client_data['insights'])) {
        $facts['hub_insights'] = $client_data['insights'];
      }
      
      // Merge analytics data (GA4, Search Console, Lighthouse, Competitor) - CRITICAL for analytics questions
      // ALWAYS merge analytics data if present, regardless of use_hub_data flag
      if (isset($client_data['analytics']) && is_array($client_data['analytics']) && !empty($client_data['analytics'])) {
        $facts['analytics'] = $client_data['analytics'];
        error_log('[Luna Widget] ✅ Merged analytics data from client_data. Has GA4: ' . (isset($client_data['analytics']['ga4']) ? 'yes' : 'no') . ', Has Search Console: ' . (isset($client_data['analytics']['search_console']) ? 'yes' : 'no') . ', Has Lighthouse: ' . (isset($client_data['analytics']['lighthouse']) && is_array($client_data['analytics']['lighthouse']) && count($client_data['analytics']['lighthouse']) > 0 ? 'yes (' . count($client_data['analytics']['lighthouse']) . ' reports)' : 'no'));
        
        // Debug: Log GA4 metrics if present
        if (isset($client_data['analytics']['ga4']) && is_array($client_data['analytics']['ga4'])) {
          $ga4 = $client_data['analytics']['ga4'];
          error_log('[Luna Widget] GA4 data: users=' . (isset($ga4['metrics']['totalUsers']) ? $ga4['metrics']['totalUsers'] : 'N/A') . ', sessions=' . (isset($ga4['metrics']['sessions']) ? $ga4['metrics']['sessions'] : 'N/A') . ', pageViews=' . (isset($ga4['metrics']['screenPageViews']) ? $ga4['metrics']['screenPageViews'] : 'N/A'));
        }
      } else {
        error_log('[Luna Widget] ⚠️ No analytics data in client_data. client_data keys: ' . (is_array($client_data) ? implode(', ', array_keys($client_data)) : 'not array'));
      }
    }
    ...
    // Build OpenAI messages with enhanced facts (including Hub data if available)
    $messages = luna_openai_messages_with_facts($pid, $prompt, $facts, false, $is_composer, $use_hub_data);
...
function luna_widget_get_html(WP_REST_Request $req) {
  ...
  // Position now handled by Supercluster styling in styles.css
  $pos_css = '';
  $panel_anchor = '';
...
function luna_widget_render_widget() {
  ...
  ?>
  <div id="luna-widget-container" class="luna-widget" style="display:none;">
  ...
  <style>
    ...
    /* Position styling moved to Supercluster styles.css */
